...
export function ClientFilter({
  ...
  for (const client of [...(clients.data?.observed?.items ?? []), ...(clients.data?.items ?? [])]) {
    if (client.address) {
  ...
  const options = known.filter((c) => `${c.name} ${c.address}`.toLowerCase().includes(term));
  const choices = term ? options : [{ address: "", name: "All clients" }, ...options];
  // A literal address remains usable even when the client inventory is unavailable.
  if (term && /^[\da-f:.%]+$/i.test(term) && /[.:]/.test(term) && !byAddress.has(search.trim())) {
    ...
    >
      <Input
        ...
        onKeyDown={(e) => {
          if (e.key === "ArrowDown" || e.key === "ArrowUp") {
            ...
            e.preventDefault();
            const choice = choices[active] ?? (choices.length === 1 ? choices[0] : undefined);
      ...
      {open && (
        <div className="absolute top-full z-20 mt-1 w-full min-w-0 rounded-md border border-border bg-background p-1 shadow-md">
          <ul id={id} role="listbox" aria-label="Clients" className="max-h-64 overflow-y-auto">
            {choices.map((client, i) => (
              ...
              >
                <span className="block truncate">{client.name || client.address}</span>
          ...
          {choices.length === 0 && (
            <p className="px-2.5 py-2 text-xs text-muted-foreground">
              {clients.loading ? "Loading clients…" : "No matching clients. Enter an IP address."}
            </p>
          )}

...
export function BootstrapSettings({
  ...
  const configured = (settings.config?.dns as Row | undefined)?.bootstrap_dns as
    | string[]
    | undefined;
  ...
  return (
    <details className="mb-5 min-w-0 rounded-lg border border-border bg-background p-5">
      <summary className="cursor-pointer text-sm font-medium">Advanced: bootstrap DNS</summary>
      <form
        ...
        onSubmit={async (event) => {
          ...
          if (!servers.length || servers.length > 16) {
            setError(new Error("Enter 1 to 16 bootstrap DNS servers, or use automatic defaults."));
            return;
          }
          if (!revision) return;
          setBusy(true);
          try {
            await api.edit(revision, [{ path: ["dns", "bootstrap_dns"], value: servers }]);
            setDraft(undefined);
      ...
      >
        <p className="max-w-[75ch] text-xs text-muted-foreground">
          Bootstrap DNS finds the IP addresses of encrypted DNS servers. These lookups use standard,
          unencrypted DNS and only ask for server hostnames, not the websites you visit.
        </p>
        <p className="text-xs text-muted-foreground">
          Automatic defaults: 1.1.1.1:53 and 9.9.9.9:53. Override these if your network needs
          different resolvers.
        </p>
        <div className="space-y-2">
          ...
          <textarea
            ...
            value={draft ?? (configured?.length ? configured : defaults).join("\n")}
            onChange={(event) => change(event.target.value)}
          />
          <p id="bootstrap-help" className="text-xs text-muted-foreground">
            One IP address and port per line, up to 16. For IPv6, use [address]:port. Do not use
            URLs or dimsum’s own listening address.
          </p>
        </div>

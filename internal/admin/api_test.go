...
func TestUnixPermissionsAndCLIParity(t *testing.T) {
	...
	out.Reset()
	body = mutation(t, store, map[string]any{"scope": "client", "id": "address:192.0.2.4", "promote_id": "workstation", "fields": []any{map[string]any{"path": []string{"blocking"}, "value": false}}})
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "client-policy-preview", body}, &out, &stderr), stderr.String())
	assert.Contains(t, out.String(), `"workstation"`)
	out.Reset()
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "patch", "client-policy", body}, &out, &stderr), stderr.String())
	out.Reset()
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "client-policy", "--query", "scope=client&id=workstation"}, &out, &stderr), stderr.String())
	assert.Contains(t, out.String(), `"filtering":false`)
	out.Reset()
	body = mutation(t, store, map[string]any{"scope": "network", "subscribe": []any{map[string]any{"id": "work", "url": "builtin://work-compatibility", "dialect": "dns-adblock", "domain_kind": "suffix", "enabled": true}}})
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "patch", "client-policy", body}, &out, &stderr), stderr.String())
	out.Reset()
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "builtin-list", "work"}, &out, &stderr), stderr.String())
	assert.JSONEq(t, request(server.LocalHandler(), "GET", "/api/v1/builtin-lists/work", "").Body.String(), out.String())
	out.Reset()
	body = mutation(t, store, map[string]any{"action": "add", "domain": "custom.example"})
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "patch", "builtin-lists/work", body}, &out, &stderr), stderr.String())
	assert.Equal(t, []string{"custom.example"}, store.Snapshot().Config().Lists[0].BuiltinOverrides.Additions)
	out.Reset()
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "device-rules"}, &out, &stderr), stderr.String())
	assert.JSONEq(t, request(server.LocalHandler(), "GET", "/api/v1/device-rules", "").Body.String(), out.String())
	out.Reset()
	body = mutation(t, store, map[string]any{"action": "save", "id": "example", "rule": map[string]any{"id": "example", "name": "Example device", "domains": []string{"device.example"}}})
	require.Equal(t, 0, cli.Run(t.Context(), []string{"--socket", socket, "patch", "device-rules", body}, &out, &stderr), stderr.String())
	assert.Equal(t, "example", store.Snapshot().Config().Naming.DNSGuesses.Custom[0].ID)
	out.Reset()

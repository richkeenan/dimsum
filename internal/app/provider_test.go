...
func TestHistoryProviderRealSQLitePresentationAndConsistentDefaults(t *testing.T) {
	...
	assert.Equal(t, rankedClient{Address: "192.0.2.10", Name: "Office", NameSource: "override", Count: "4"}, rankings.Clients[0])
	rankingJSON, err := json.Marshal(rankings.Clients[0])
	require.NoError(t, err)
	assert.Contains(t, string(rankingJSON), `"name_source":"override"`)

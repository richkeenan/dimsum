...
Collection update tools use relative paths starting with a zero-based index string, for example update_filter_lists uses ["1","enabled"], not ["lists","1","enabled"]. Settings and staged configuration edits use root-relative paths. Read current items and saved_revision first. Inspect field_errors for request corrections. explain_domain accepts names copied from query logs, including decimal escapes; collect independent diagnostic results separately so one error does not hide successful calls.

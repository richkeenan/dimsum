...
func TestToolsDerivedFromSpec(t *testing.T) {
	...
	listSchema, err := json.Marshal(tools["update_filter_lists"].InputSchema)
	require.NoError(t, err)
	assert.Contains(t, string(listSchema), "relative to the collection")
	assert.Contains(t, tools["update_filter_lists"].Description, `["1","enabled"]`)
	explainSchema, err := json.Marshal(tools["explain_domain"].InputSchema)
	require.NoError(t, err)
	assert.Contains(t, string(explainSchema), "query logs")
